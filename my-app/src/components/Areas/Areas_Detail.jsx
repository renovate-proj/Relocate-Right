...
import { MapPin, Star, TrendingUp, TrendingDown, Home, Train, ShoppingBag, Heart, Share2, Map, Shield, DollarSign, Bus, School, Hospital, TreePine, Utensils, Coffee, ChevronRight, Users, Clock, ArrowRight, X, Menu } from 'lucide-react';
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className={`p-2 rounded-lg ${getScoreBgColor(score)}`}>
            <Icon className={`w-4 h-4 sm:w-5 sm:h-5 ${getScoreColor(score)}`} />
          </div>
          <div>
            <h3 className="font-semibold text-gray-900 text-sm sm:text-base">{title}</h3>
            <p className={`text-xl sm:text-2xl font-bold ${getScoreColor(score)}`}>{score}/10</p>
          </div>
        </div>
      </div>
        ...
        <div className="mt-4 space-y-2">
          {Object.entries(breakdown).map(([key, value]) => (
            <div key={key} className="flex items-center justify-between">
              <span className="text-xs sm:text-sm text-gray-600 capitalize">{key}</span>
              <div className="flex items-center gap-2">
                ...
                {typeof value === 'number' && <span className="text-xs sm:text-sm font-medium text-gray-700 w-8">{value}</span>}
              </div>
            </div>
          ))}
        </div>
...
const CostItem = ({ label, value, comparison, isRent = false }) => {
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <span className="font-semibold text-sm sm:text-base text-gray-900">{typeof value === 'number' ? `₹${value.toLocaleString()}` : value}</span>
        {isRent && comparison && (
...
export default function AreaDetailPage({ slug }) {
      ...
      try {
        ...
        if (!data) {
          ...
          const transformed = {
            ...
            metrics: {
              safety: { score: data.safety_score || 0, breakdown: { police: 8, lighting: 8 } },
            ...
            costOfLiving: {
              ...
              mumbaiAverage: { '1bhk': 1500, '2bhk': 2500, '3bhk': 3500 }
            },
            amenitiesCount: { schools: 45, hospitals: 12, parks: 20, restaurants: 150, cafes: 40, gyms: 25 },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-sm sm:text-base text-gray-600">The requested location "{slug}" could not be found.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex gap-1">
        {[...Array(5)].map((_, i) => (
          <Star
            key={i}
            className={`w-4 h-4 sm:w-5 sm:h-5 ${i < stars ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 sm:gap-3 mb-1">
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 truncate">{neighborhood.name}</h1>
              ...
              <div className="flex items-center gap-2 text-gray-600">
                ...
                <span className="text-xs sm:text-sm truncate">{neighborhood.state}, {neighborhood.country}</span>
              </div>
            </div>
            ...
            <div className="flex sm:hidden gap-2">
              <button
                ...
                className={`flex-1 px-3 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 flex items-center justify-center gap-2 transition-colors ${isSaved ? 'text-red-500 border-red-200 bg-red-50' : ''}`}
              >
                <Heart className={`w-4 h-4 ${isSaved ? 'fill-current' : ''}`} />
              ...
              <button
                ...
                className="flex-1 px-3 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 flex items-center justify-center gap-2 transition-colors"
              >
                <Share2 className="w-4 h-4" />
      ...
      <div className="relative h-48 sm:h-64 md:h-80 lg:h-96 bg-gray-300">
        <img
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 lg:p-8">
          <div className="max-w-7xl mx-auto">
            <div className="bg-white rounded-lg p-4 sm:p-6 shadow-xl inline-block">
              <div className="flex items-center gap-3 sm:gap-4">
                <div>
                  <p className="text-xs sm:text-sm text-gray-600 mb-1">Overall Score</p>
                  <p className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">{neighborhood.overallScore}</p>
                </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <div className="flex gap-4 sm:gap-8 overflow-x-auto scrollbar-hide">
            {tabs.map(tab => (
              <button
                ...
                className={`pb-3 sm:pb-4 px-1 font-medium transition-colors relative whitespace-nowrap text-sm sm:text-base ${activeTab === tab.id
                  ? 'text-blue-600'
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 sm:space-y-8">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
              <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">About {neighborhood.name}</h2>
              <p className="text-sm sm:text-base text-gray-700 leading-relaxed">{neighborhood.description}</p>
            </div>

            {/* Metrics Grid - Responsive */}
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">Key Metrics</h2>
            ...
            {/* Photos Gallery - Responsive */}
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">Photos</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4">
                {neighborhood.photos && neighborhood.photos.map((photo, idx) => (
                  <div key={idx} className="relative h-32 sm:h-48 bg-gray-200 rounded-lg overflow-hidden group cursor-pointer">
        ...
        {activeTab === 'cost' && (
          <div className="space-y-4 sm:space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              ...
              {/* Utilities */}
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">Monthly Utilities</h2>
                <div className="space-y-1">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 lg:col-span-2">
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">Other Monthly Costs</h2>
        ...
        {activeTab === 'transport' && (
          <div className="p-4 sm:p-6 bg-white rounded-lg shadow-sm">
            <h3 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4">Transport Stats</h3>
        ...
        {activeTab === 'amenities' && (
          <div className="p-4 sm:p-6 bg-white rounded-lg shadow-sm">
            <h3 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4">Amenities Stats</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 bg-gray-50 rounded">
        ...
        {activeTab === 'reviews' && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-0 mb-4 sm:mb-6">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900">User Reviews</h2>
              <button
            ...
            {showReviewForm && (
              <form onSubmit={handleReviewSubmit} className="mb-6 sm:mb-8 p-4 bg-gray-50 rounded-lg">
                <div className="mb-4">
                  <label className="block text-gray-700 font-bold mb-2 text-sm sm:text-base">Rating</label>
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star
                          className={`w-6 h-6 sm:w-8 sm:h-8 ${star <= newReview.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                        />
                      </button>
                    ))}
                  </div>
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition disabled:opacity-50 text-sm sm:text-base"
                >
            ...
            {reviewsLoading ? (
              <p className="text-center text-gray-500 text-sm sm:text-base">Loading reviews...</p>
            ) : reviews.length > 0 ? (
              <div className="space-y-4 sm:space-y-6">
                {reviews.map((review) => (
                  <div key={review.id} className="border-b border-gray-100 pb-4 sm:pb-6 last:border-0 last:pb-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-semibold text-gray-900 text-sm sm:text-base">{review.user?.full_name || 'Anonymous User'}</span>
              ...
              <div className="text-center py-8">
                <p className="text-gray-500 mb-4 text-sm sm:text-base">No reviews yet.</p>
                {!showReviewForm && (
        ...
        {/* Add to Comparison CTA - Mobile Optimized */}
        <div className="mt-6 sm:mt-8 bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg p-4 sm:p-6 lg:p-8 text-white">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h3 className="text-lg sm:text-xl lg:text-2xl font-bold mb-2">Compare with Other Areas</h3>
              <p className="text-sm sm:text-base text-blue-100">Add {neighborhood.name} to your comparison list to see how it stacks up against other neighborhoods.</p>
            </div>
            <button
              onClick={() => {
                if (!isInComparison) {
              ...
              className={`w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold transition-colors whitespace-nowrap text-sm sm:text-base ${isInComparison
...
export function SampleNeighborhoodPages() {
  ...
  return (
    <div>
      {/* Navigation to switch between neighborhoods - Mobile Optimized */}
      <div className="bg-gray-900 text-white p-3 sm:p-4 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="font-bold mb-2 sm:mb-0 text-sm sm:text-base">Sample Neighborhoods:</h2>
          <div className="flex gap-2 overflow-x-auto scrollbar-hide">
            {slugs.map(slug => (
              <button
                key={slug}
                onClick={() => setCurrentSlug(slug)}
                className={`px-3 sm:px-4 py-2 rounded-lg transition-colors whitespace-nowrap text-xs sm:text-sm ${currentSlug === slug
                  ? 'bg-blue-600'
